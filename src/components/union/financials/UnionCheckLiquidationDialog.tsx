...
interface UnionCheckLiquidationDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  clinicId: string;
}

export function UnionCheckLiquidationDialog({
  open,
  onOpenChange,
  clinicId,
}: UnionCheckLiquidationDialogProps) {
  ...
  // Fetch expenses by check number from union_financial_transactions
  const { data: expenses, isLoading: loadingExpenses } = useQuery({
    queryKey: ["union-expenses-by-check", clinicId, searchedCheckNumber],
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          cash_register:union_cash_registers(id, name, bank_name)
        `)
  ...
  const selectedTotal = useMemo(() => {
    ...
    return expenses
      ...
      .reduce((sum, e) => sum + Number(e.net_value || e.amount), 0);
  ...
  const liquidateMutation = useMutation({
    mutationFn: async () => {
      ...
      // Log to union_audit_logs
      const { error: auditError } = await supabase
        .from("union_audit_logs")
        .insert({
          clinic_id: clinicId,
          user_id: session?.user?.id,
          action: "check_liquidation",
          entity_type: "union_financial_transactions",
          details: {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["union-expenses-by-check"] });
      queryClient.invalidateQueries({ queryKey: ["union-cash-registers"] });
      queryClient.invalidateQueries({ queryKey: ["union-cash-flow"] });
