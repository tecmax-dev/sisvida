...
export const ExamRequestPrint = forwardRef<HTMLDivElement, ExamRequestPrintProps>(
  ({ clinic, patient, professional, examRequest, clinicalIndication, date, settings }, ref) => {
    ...
    const title = settings?.exam_request_title || 'SOLICITAÇÃO DE EXAMES';
    ...
    return (
      <div ref={ref} className={`${paperDimensions.padding} bg-white text-black ${paperDimensions.minHeight}`} style={{ width: paperDimensions.width }}>
        ...
        <div className="mb-8">
          <h3 className="font-semibold text-lg mb-4">Exames Solicitados:</h3>
          <div className="whitespace-pre-wrap text-base leading-relaxed pl-4">
            {examRequest}
          </div>
        </div>
        ...
        {clinicalIndication && (
          <div className="mb-16">
            <h3 className="font-semibold text-lg mb-2">Indicação Clínica:</h3>
            <p className="text-base text-gray-700">{clinicalIndication}</p>
          </div>
        )}
...
ExamRequestPrint.displayName = "ExamRequestPrint";
