...
export function SuppliersPanel({ clinicId }: SuppliersPanelProps) {
  ...
  const { data: suppliers, isLoading } = useQuery({
    queryKey: ["suppliers", clinicId, showInactive],
    queryFn: async () => {
      let query = supabase
        .from("suppliers")
        .select("*")
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Gerencie os fornecedores para lançamento de despesas
          </p>
        </div>
        <Button
          onClick={() => {
