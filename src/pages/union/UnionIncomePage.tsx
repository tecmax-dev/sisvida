...
export default function UnionIncomePage() {
  ...
  const pendingIncome = transactions?.filter((t) => t.status === "pending").reduce((sum, t) => sum + Number(t.amount), 0) || 0;
  const receivedIncome = transactions?.filter((t) => t.status === "paid").reduce((sum, t) => sum + Number(t.amount), 0) || 0;
