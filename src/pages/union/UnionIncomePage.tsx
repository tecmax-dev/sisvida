...
export default function UnionIncomePage() {
  ...
  const { canManageIncome } = useUnionPermissions();
  ...
  const { data: transactions, isLoading } = useQuery({
    queryKey: ["union-financial-transactions", clinicId, "income"],
    queryFn: async () => {
  ...
  const { data: categories } = useQuery({
    queryKey: ["union-financial-categories", clinicId, "income"],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const deleteMutation = useMutation({
    mutationFn: async (transaction: any) => {
      if (transaction.status === "paid") {
        throw new Error("Não é possível excluir receitas já recebidas. Use o estorno.");
      }
  ...
  const totalIncome = processedTransactions.reduce(
    (sum, t) => sum + Number(t.net_value || t.amount),
    0
  );
  const pendingIncome = processedTransactions
    .filter((t) => t.effectiveStatus === "pending")
    .reduce((sum, t) => sum + Number(t.net_value || t.amount), 0);
  const overdueIncome = processedTransactions
    .filter((t) => t.effectiveStatus === "overdue")
    .reduce((sum, t) => sum + Number(t.net_value || t.amount), 0);
  const receivedIncome = processedTransactions
                ...
                <TableBody>
                  {filteredTransactions.map((transaction) => (
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>{transaction.cash_register?.name || "-"}</TableCell>
                      <TableCell>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {(transaction.effectiveStatus === "pending" ||
                                ...
                                <>
                                  <DropdownMenuItem
                                    onClick={() => receiveMutation.mutate(transaction)}
                                  >
