-- Update the validate_appointment_cpf_restriction function to use patient's individual limit
...
-- Update the validate_dependent_appointment_limit function similarly
...
COMMENT ON FUNCTION public.validate_appointment_cpf_restriction() IS 'Validates appointment limits per CPF per month per professional. Uses patient individual limit (max_appointments_per_month) if set, otherwise falls back to clinic default (max_appointments_per_cpf_month).';