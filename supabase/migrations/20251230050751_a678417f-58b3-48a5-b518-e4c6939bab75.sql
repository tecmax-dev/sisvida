...
DECLARE
  ...
  month_end DATE;
BEGIN
  -- Skip validation for cancellations, completed and no_show appointments
  IF NEW.status IN ('cancelled', 'completed', 'no_show') THEN
    RETURN NEW;
  END IF;

  -- Skip validation on UPDATE if status is changing to cancelled/completed/no_show
  ...
  -- Count existing ACTIVE appointments in the same month for the same professional
  -- Exclude cancelled, completed, and no_show from the count
