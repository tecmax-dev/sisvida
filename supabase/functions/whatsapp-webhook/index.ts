...
// ==========================================
// MULTIATTENDANCE MODULE INTEGRATION
// ==========================================

async function handleMultiattendanceMessage(
  supabase: any,
  clinicId: string,
  phone: string,
  messageText: string,
  ...
  console.log(`[multiattendance] Processing message for clinic ${clinicId} from ${phone}`);
  
  try {
    // 1. Find or create contact
    let contact = await findOrCreateWhatsAppContact(supabase, clinicId, phone);
    
    if (!contact) {
      console.error('[multiattendance] Failed to create/find contact');
      return;
    }
    
    // 2. Find open ticket or create new one
    let ticket = await findOrCreateWhatsAppTicket(supabase, clinicId, contact.id);
    
    if (!ticket) {
      console.error('[multiattendance] Failed to create/find ticket');
      return;
    }
    
    // 3. Add message to ticket
    const { error: messageError } = await supabase
      .from('whatsapp_ticket_messages')
      .insert({
        ticket_id: ticket.id,
        sender_type: 'contact',
    ...
    if (messageError) {
      console.error('[multiattendance] Error inserting message:', messageError);
    }
    
    // 4. Update ticket last_message_at
    await supabase
    ...
    console.log(`[multiattendance] Message saved to ticket ${ticket.protocol}`);
    
  } catch (error) {
    console.error('[multiattendance] Error handling message:', error);
  }
}
...
async function findOrCreateWhatsAppContact(
  ...
  if (error) {
    console.error('[multiattendance] Error creating contact:', error);
    return null;
  }
...
async function findOrCreateWhatsAppTicket(
  supabase: any,
  clinicId: string,
  contactId: string
): Promise<{ id: string; protocol: string; unread_count: number } | null> {
  ...
  const { data: existingTicket } = await supabase
    ...
    .in('status', ['pending', 'open', 'waiting', 'bot'])
  ...
  // Create new ticket (protocol is auto-generated by trigger)
  ...
  if (error) {
    console.error('[multiattendance] Error creating ticket:', error);
    return null;
  }
  ...
  try {
    ...
    if (!confirmResult.handled) {
      ...
      if (configData) {
        ...
        // ============================================
        // CHECK FOR MULTIATTENDANCE MODULE FIRST
        // ============================================
        const { data: hasMultiattendance } = await supabase
          .from('clinic_addons')
          .select(`
            id,
            status,
        ...
        if (hasMultiattendance) {
          console.log(`[webhook] Clinic ${clinicId} has multiattendance addon - routing to tickets`);
          
          // Route message to multiattendance module
          await handleMultiattendanceMessage(supabase, clinicId, phone, messageText, payload);
          
          return new Response(
            JSON.stringify({ success: true, handled_by: 'multiattendance' }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }

        // ============================================
        // FALLBACK TO BOOKING FLOW
        // ============================================
